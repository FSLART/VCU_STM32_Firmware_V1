...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan) {
    ...
    // Handle all CAN buses in the callback for real-time processing
    if (hcan == &hcan1) {
        // CAN1 - Data Bus
        if (HAL_CAN_GetRxMessage(&hcan1, CAN_RX_FIFO0, &RxHeader, RxData) == HAL_OK) {
            decode_DATA_DB(RxHeader, RxData);  // Process data bus messages
        }
    } else if (hcan == &hcan2) {
        // CAN2 - Powertrain Bus
        if (HAL_CAN_GetRxMessage(&hcan2, CAN_RX_FIFO0, &RxHeader, RxData) == HAL_OK) {
            decode_PWT_DB(RxHeader, RxData);  // Process powertrain messages
        }
    } else if (hcan == &hcan3) {
        // CAN3 - Autonomous Bus
        if (HAL_CAN_GetRxMessage(&hcan3, CAN_RX_FIFO0, &RxHeader, RxData) == HAL_OK) {
            decode_AUTO_DB(RxHeader, RxData);  // Process autonomous messages
        }
    }
}
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
    ...
    /* USER CODE BEGIN 3 */
        ...
        // CAN processing now handled in interrupt callback for better real-time performance
        ...
        if (current_tick - previous_tick >= 10) {
            ...
            can_send_st_wheel_data(&hcan2, result.percentage, vcu.brake_pressure, myHV500.Actual_TempController, myHV500.Actual_TempMotor, bms.instant_voltage, bms.soc, result.error_type, myHV500.Actual_InputVoltage, myHV500.Actual_ERPM, vcu.ignition_switch_signal, vcu.r2d_toggle_signal);
